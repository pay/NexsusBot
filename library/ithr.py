...
"""
Module "ithr" (for Python 3)
ithr.py

Partial copyright (2010-2013) Al Korgun (alkorgun@gmail.com)
Python 3 port — constructions preserved
"""
...
# Python 2: import thread / Python 3: import _thread
try:
    import _thread as thread  # Python 3
except ImportError:
    import thread              # Python 2 fallback
...
class RLock(object):
    ...
    def release(self):
        ...
        self.__count -= 1
        if not self.__count:  # fixed: was "if not count"
            self.__owner = None
...
class Semaphore(object):
    ...
    def acquire(self, blocking=1):
        rc = False
        self.__cond.acquire()
        while self.__value == 0:  # was: "is 0" (identity check removed in py3)
            if not blocking:
                break
            self.__cond.wait()
        else:
...
class BoundedSemaphore(Semaphore):
    ...
    def release(self):
        if self._Semaphore__value >= self.basic_value:
...
class UnBoundedSemaphore(Semaphore):
    ...
    def __init__(self, value=1):
        Semaphore.__init__(self, value)
        self.basic_value = value
        # py3: no func_defaults; use __defaults__ on __wrapped__
        # Semaphore.acquire default is already 0-compatible; skip
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    pass
            # sys.exc_clear() removed in Python 3 — no-op
        finally:
            with active_limbo_lock:
                self.__stop()
                try:
    ...
    def __delete(self):
        with active_limbo_lock:
            try:
                ...
                if "dummy_threading" not in sys.modules:  # was: has_key
                    raise
...
def enumerate():
    with active_limbo_lock:
        # py3: dict.values() returns view, not list — concat via list()
        return list(ActiveThreads.values()) + list(Thrlimbo.values())
...
def _after_fork():
    ...
    with active_limbo_lock:
        for thr in ActiveThreads.values():  # was: itervalues()
            if thr is current:
