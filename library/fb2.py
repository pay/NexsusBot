...
# Fix: htmlentitydefs → html.entities (Python 3)
...
# Fix: .iteritems() → .items(), unichr() → chr()
for Name, Numb in htmlentitydefs.name2codepoint.items():
	edefs[Name] = chr(Numb)
...
compile_ehtmls  = compile__(r"&(#?[xX]?(?:[0-9a-fA-F]+|\w{1,8}));")   # Fix: raw string
compile_stitle  = compile__(r"<title>((?:.|\s)+?)</title>", 16)          # Fix: raw string
compile_subtitle = compile__(r"<subtitle>((?:.|\s)+?)</subtitle>", 16)   # Fix: raw string
...
def get_text(data, s0, s2, s1=r"(?:.|\s)+"):   # Fix: raw string default
...
get_enc = lambda data: get_text(data, 'encoding="', r'"\?')   # Fix: raw string
...
def get_data(data):
	...
	if desc:
		...
		if creator:
			...
			author = None
		comp   = compile__(r'<binary.+?content-type="image/(.+?)".*?>((?:.|\s)+?)</binary>', 16)  # Fix: raw
		...
		if sequence:
			seq1 = get_text(sequence, 'name="', '"')
			seq2 = get_text(sequence, 'number="', '"')
			if seq2:
				ls = seq2.split(".")
				try:
					seq2 = int(ls[-1])
	...
	if body:
		comp = compile__(r"<section.*?>((?:.|\s)+?)</section>", 16)   # Fix: raw
...
def make(body, Name, author=None, year=None, genre=None, seq1=None, seq2=0,
         cover=None, annt=None, lang="en", bs="fb2.py %s" % __version__, User=None):
...
<description>''']
	...
	if author:
		...
		if len(ls) == 1:
			...
			data.append("<first-name>%s</first-name><middle-name>%s</middle-name><last-name>%s</last-name>" % (
				first, str.join(chr(32), ls), last))
		data.append("</author>")
